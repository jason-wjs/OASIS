...
class MultiImageReader:
    ...
    def __init__(self):
        ...
        self.shms = {}  # image_name -> SharedMemory
        self._warned_not_found = set()  # image names already warned about missing shm
        ...
        for image_name in image_names:
            try:
                ...
                if shm_name not in self.shms:
                    try:
                        self.shms[shm_name] = shared_memory.SharedMemory(name=shm_name)
                        self._warned_not_found.discard(image_name)
                    except FileNotFoundError:
                        if image_name not in self._warned_not_found:
                            print(f"[MultiImageReader] Shared memory {get_shm_name(image_name)} not found, waiting...")
                            self._warned_not_found.add(image_name)
                        continue
        ...
        try:
            ...
            if shm_name not in self.shms:
                try:
                    ...
                    if image_name not in self._warned_not_found:
                        print(f"[MultiImageReader] Shared memory {shm_name} not found, waiting...")
                        self._warned_not_found.add(image_name)
                    return None
