...
@MotionsSceneCfgFactory.register("pick_tennis")
class TennisCfg(InteractiveSceneCfg, RandomizableSceneMixin): # inherit from the interactive scene configuration class
    ...
    white_basket = RigidObjectCfg(
        prim_path="/World/envs/env_.*/white_basket",
        init_state=RigidObjectCfg.InitialStateCfg(pos=[-2.8, -2.05, 0.75],
                                                  rot=[0, 1, 0, 0]),
        spawn=UsdFileCfg(
            usd_path=f"{ASSET_ROOT}/objects/white_basket/white_basket.usd",
    ...
    tennis = RigidObjectCfg(
        prim_path="/World/envs/env_.*/tennis",
        init_state=RigidObjectCfg.InitialStateCfg(pos=[-2.5, -2.2, 0.8],
                                                  rot=[0, 1.0, 0, 0]),
        spawn=UsdFileCfg(
            usd_path=f"{ASSET_ROOT}/objects/tennis/tennis.usd",
            rigid_props=sim_utils.RigidBodyPropertiesCfg(
                disable_gravity=False,
                retain_accelerations=False,
            ),
